  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className="p-4">
          <h3 className="text-lg font-semibold text-gray-800 truncate">{miss.name}</h3>
          <p className="text-gray-600 text-sm mb-3">Miss {miss.region}</p>
      ...
      {/* Badge de rang */}
      {rank && (
        <div className="absolute top-2 left-2 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-full px-3 py-1 text-sm font-bold z-10 shadow-lg">
          {rank}
        </div>
      )}
      ...
      {showRemoveButton && (
        <button 
          onClick={(e) => { e.stopPropagation(); onRemove(); }}
          className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 z-10 shadow-lg transition-colors"
        >
          ×
        </button>
      )}
